...
Main file for bifacial runs, yearly enhanced albedo case (set to 0.6)
"""
...
if site=='Bondville_IL':
    lat=40.05192; lon=-88.37309; ele=230; albedo=0.6
elif site=='Boulder_CO':
    lat=40.12498; lon=-105.2368; ele=1689; albedo=0.6
elif site=='Desert_Rock_NV':
    lat=36.62373; lon=-116.01947; ele=1007; albedo=0.6
elif site=='Fort_Peck_MT':
    lat=48.30783; lon=-105.10170; ele=634; albedo=0.6
elif site=='Goodwin_Creek_MS':
    lat= 34.2547; lon=-89.8729; ele=98.0; albedo=0.6 
elif site=='Penn_State_PA':
    lat=40.72012; lon=-77.93085; ele=376; albedo=0.6
elif site=='Sioux_Falls_SD':
    lat=43.73403; lon=-96.62328; ele=473; albedo=0.6    
...
for site in sites:
    if site=='Bondville_IL':
        lat=40.05192; lon=-88.37309; ele=230; albedo=0.6; tiltopt=33
    elif site=='Boulder_CO':
        lat=40.12498; lon=-105.2368; ele=1689; albedo=0.6; tiltopt=37
    elif site=='Desert_Rock_NV':
        lat=36.62373; lon=-116.01947; ele=1007; albedo=0.6; tiltopt=35
    elif site=='Fort_Peck_MT':
        lat=48.30783; lon=-105.10170; ele=634; albedo=0.6; tiltopt=40
    elif site=='Goodwin_Creek_MS':
        lat= 34.2547; lon=-89.8729; ele=98.0; albedo=0.6 ; tiltopt=30
    elif site=='Penn_State_PA':
        lat=40.72012; lon=-77.93085; ele=376; albedo=0.6; tiltopt=32
    elif site=='Sioux_Falls_SD':
        lat=43.73403; lon=-96.62328; ele=473; albedo=0.6; tiltopt=38

    tilts=[10,25,tiltopt]
    for tiltang in tilts:
        #tiltang=10
        f = open('../'+site+'_tilt'+str(tiltang)+'_output_bifacial_06albedo.txt', 'w')
        ...
        for year in range(2011,2021):
            ...
            metyear.to_csv('../met'+site+str(year)+'_tilt'+str(tiltang)+'_bifacial_06albedo.csv')
            pd.DataFrame(irrad.effective_irradiance).to_csv('../poa'+site+str(year)+'_tilt'+str(tiltang)+'_bifacial_06albedo.csv')
...
for site in sites:
    ...
    f = open('../'+site+'_tracking_output_bifacial_06nalbedo.txt', 'w')
    ...
    for year in range(2011,2021):
        ...
        #mc.run_model(metyear)
                # save data per site per year
        metyear.to_csv('../met'+site+str(year)+'_tracking_bifacial_06albedo.csv')
        pd.DataFrame(irrad.effective_irradiance).to_csv('../poa'+site+str(year)+'_tracking_bifacial_06albedo.csv')
...
''' continue after saving yearly data'''
...
for site in sites:
    ...
    tilts=[10,25,tiltopt]
    for tiltang in tilts:
        met=pd.DataFrame(); met.is_copy = False
        poa=pd.DataFrame(); poa.is_copy = False
        f = open('../out/overNEC_'+site+'_tilt'+str(tiltang)+'_output_bifacial_06albedo.txt', 'a')
        for year in range(2011,2021):
            metyear=pd.read_csv('../met'+site+str(year)+'_tilt'+str(tiltang)+'_bifacial_06albedo.csv',index_col=0,parse_dates=[0])
            poayear=pd.read_csv('../poa'+site+str(year)+'_tilt'+str(tiltang)+'_bifacial_06albedo.csv',index_col=0,parse_dates=[0])
        ...
        iweirdtemp=np.where(np.logical_or(met.temp_air<-40,met.temp_air>50))[0]
        for ii in iweirdtemp:
            met.temp_air[ii]=0.5*(met.temp_air[ii-2]+met.temp_air[ii+2])
        ...
        system = pvsystem.PVSystem(surface_tilt=tiltang, surface_azimuth=180, 
                                albedo=albedo, module_parameters=module, 
                                temperature_model_parameters={'noct':module.T_NOCT, 'module_efficiency':eta_m},
                                inverter_parameters=inverter, modules_per_string=18, strings_per_inverter=5)
        location=Location(lat, lon,altitude=ele)
        mc = ModelChain(system, location, clearsky_model='ineichen', transposition_model='perez', 
                                ...
                                temperature_model='noct_sam', losses_model='no_loss')
        ...
        np.savetxt('../out/table_Isc_'+site+'_tracking_bifacial_06albedo.csv',Isc_out,delimiter=',')
        np.savetxt('../out/table_Voc_'+site+'_tracking_bifacial_06albedo.csv',Voc_out,delimiter=',')
        np.savetxt('../out/table_Imp_'+site+'_tracking_bifacial_06albedo.csv',Imp_out,delimiter=',')
        np.savetxt('../out/table_Vmp_'+site+'_tracking_bifacial_06albedo.csv',Vmp_out,delimiter=',')
        ...
        topd= {'site': [site], 'tilt': [tiltang], 'type':['BF, enhanced'], 
...
for site in sites:
    ...
    met=pd.DataFrame()
    poa=pd.DataFrame()
    f = open('../out/overNEC_'+site+'_tracking_output_bifacial_06albedo.txt', 'a')
    for year in range(2011,2021):
        metyear=pd.read_csv('../met'+site+str(year)+'_tracking_bifacial_06albedo.csv',index_col=0,parse_dates=[0])
        poayear=pd.read_csv('../poa'+site+str(year)+'_tracking_bifacial_06albedo.csv',index_col=0,parse_dates=[0])
        #append wanted dataframes
        met=met.append(metyear)
        poa=poa.append(poayear)
    ...
    iweirdpoa=np.where(poa>4000)[0]
    for ii in iweirdpoa:
        poa.effective_irradiance[ii]=0.5*(poa.effective_irradiance[ii-2]+poa.effective_irradiance[ii+2])
    ...
    topd= {'site': [site], 'tilt': ['HSAT'], 'type':['BF, enhanced'], 
              'min_Isc_overnec':[np.sum(Iscm0_1min/5>nec125)[0]], 
              'freq_Isc_overnec':[over_totalfreq],
              'dayfreq_Isc_overnec':[over_totaldaytime],
              'nevents_Isc_overnec':[np.max(Isc_overI_dt)],
...
outpd.to_csv('../out/summary_BFenhanced_fixedtilt.csv',sep=',')
